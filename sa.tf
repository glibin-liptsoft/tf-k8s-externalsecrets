resource "yandex_iam_service_account" "default" {
  count       = var.cluster_secret_store_yandexlockbox == true || var.cluster_secret_store_yandexcertificate == true ? 1 : 0
...
locals {
  service_account_key = var.cluster_secret_store_yandexlockbox == true || var.cluster_secret_store_yandexcertificate == true ? jsonencode({
